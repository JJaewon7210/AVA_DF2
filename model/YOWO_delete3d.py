...
class YOWO_CUSTOM(YOWO):
    def __init__(self, cfg):
        ...
        self.load_pretrain()
        self.delete_3D_module = True
        
        if self.delete_3D_module:
            del self.backbone_3d
            del self.cfam
            del self.conv_final
    ...
    def forward(self, x):
        ...
        if self.delete_3D_module:
            out_bbox_infer = outs_infer[..., :5]
            out_bbox_pred = outs_pred[0][..., :5]
            return [out_bbox_infer, [out_bbox_pred]], [out_clo_infer, [out_clo_pred]]
        else:
