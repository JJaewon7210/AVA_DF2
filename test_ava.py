...
def test_ava(
    ...
    for batch_i, batch in  enumerate(tqdm(dataloader, desc=s)):
        ...
        t_bbox = extract_bounding_boxes(batch['boxes'])
    ...
    if plots:
        ...
        if wandb_logger and wandb_logger.wandb:
            val_batches = [wandb_logger.wandb.Image(str(f), caption=f.name) for f in sorted(save_dir.glob('test_ava*.jpg'))]
            wandb_logger.log({"Validation": val_batches})
    if wandb_images:
        wandb_logger.log({"Bounding Box Debugger/Images_ava": wandb_images})
